package com.example.aaron.firstgame;
...
/**
 * Created by Aaron on 2015-05-29.
 */
public class GamePanel extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public GamePanel (Context context){
        ...
        //make gamePanel focusabale so it can handle events
        setFocusable(true);
    }
    ...
    public void update() {
        if (player.getPlaying()) {
            ...
            //max and mind border height are update, and the border switched direction when either max or min is met
            maxBorderHeight = 30 + player.getScore()/progressDenom;
            //cap max border height so that borders can only tak eup a total of 1/2 the screen
            ...
            if(missileElapsed > (2000 - player.getScore()/4)){
                //first missile always foes down the middle
                if (missiles.size() == 0){
            ...
            long elapsed = (System.nanoTime() - smokeStartTime/100000);
            if (elapsed > 120){
            ...
            if(resetElapsed > 4500 && !newGameCreated){
                newGame();
            }
        }
    }
    ...
    public void updateTopBorder(){
        ...
        for (int i = 0; i < topborder.size(); i++){
            ...
            if (topborder.get(i).getX()<-20){
                ...
                //new border will added will have larger height
                if(topDown){
                    topborder.add(new TopBorder(BitmapFactory.decodeResource(getResources(),R.drawable.brick),
